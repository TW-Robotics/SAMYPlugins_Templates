...
static void testUnionArrayFunction(UA_Server* server){
    UA_UnionTestDataType unionTest, unionTest2;
    unionTest.switchField = UA_UNIONTESTDATATYPESWITCH_UINT32VALUE;
    unionTest.fields.uint32Value = 24;

    unionTest2.switchField = UA_UNIONTESTDATATYPESWITCH_STRINGVALUE;
    unionTest2.fields.stringValue = UA_STRING("test string");

    UA_UnionTestDataType* unionArray = (UA_UnionTestDataType*)UA_Array_new(2, &UA_TYPES_CRCL_OPCUA[UA_TYPES_CRCL_OPCUA_UNIONTESTDATATYPE]);
    unionArray[0] = unionTest;
    unionArray[1] = unionTest2;

    // Attributes for variable node
UA_VariableAttributes vattr = UA_VariableAttributes_default;

printf("\n ouh yeah testUnionFunction      3.5       \n");

UA_Variant_setArray(&vattr.value, unionArray, 2, &UA_TYPES_CRCL_OPCUA[UA_TYPES_CRCL_OPCUA_UNIONTESTDATATYPE] );
    vattr.valueRank = UA_VALUERANK_ONE_DIMENSION;
    UA_UInt32 arrayDims[1] = {2};
    vattr.arrayDimensions = arrayDims;
    vattr.arrayDimensionsSize = 1;
    vattr.displayName = UA_LOCALIZEDTEXT("locale","UNION Array TEST");
    vattr.dataType = UA_TYPES_CRCL_OPCUA[UA_TYPES_CRCL_OPCUA_UNIONTESTDATATYPE].typeId;
    printf("\n ouh yeah testUnionFunction      3.75       \n");

    UA_StatusCode retval = UA_Server_addVariableNode(server,
                           UA_NODEID_NUMERIC(1, 555444555), // new node id
                           UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER), // parent node
                           UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES), // reference type
                           UA_QUALIFIEDNAME(1, "UNION Array TEST"), // node browse name
                           UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
                           vattr, NULL, NULL);

    if(retval == UA_STATUSCODE_GOOD){
        printf("\n testUnionFunction succesfully added!! \n");
    }else{
        printf("\n testUnionFunction adding failed!! \n");
    }
}

static void testUnionFunction(UA_Server* server){
    ...
    dattr.dataType = UA_TYPES_CRCL_OPCUA[UA_TYPES_CRCL_OPCUA_UNIONTESTDATATYPE].typeId;
    dattr.valueRank = UA_VALUERANK_SCALAR;

    UA_UnionTestDataType u;
    memset(&u, 0, sizeof(UA_UnionTestDataType));
    UA_Variant_setScalar(&dattr.value, &u, &UA_TYPES_CRCL_OPCUA[UA_TYPES_CRCL_OPCUA_UNIONTESTDATATYPE]);

    UA_Server_addVariableTypeNode(server, UA_NODEID_STRING(2, "UnionVariableType"),
                                  UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
    ...
    UA_UnionTestDataType unionTest;
    unionTest.switchField = UA_UNIONTESTDATATYPESWITCH_UINT32VALUE;
    unionTest.fields.uint32Value = 24;
    ...
    vattr.dataType = UA_TYPES_CRCL_OPCUA[UA_TYPES_CRCL_OPCUA_UNIONTESTDATATYPE].typeId;
    vattr.valueRank = UA_VALUERANK_SCALAR;
    UA_Variant_setScalar(&vattr.value, &unionTest, &UA_TYPES_CRCL_OPCUA[UA_TYPES_CRCL_OPCUA_UNIONTESTDATATYPE]);

    UA_StatusCode retval = UA_Server_addVariableNode(server, UA_NODEID_STRING(1, "Union.Value"),
                              UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
                              UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                              UA_QUALIFIEDNAME(1, "Union Example"),
                              UA_NODEID_STRING(2, "UnionVariableType"), vattr, NULL, NULL);
...
static void testArrayFunction(UA_Server* server){
    UA_Int32 int1, int2;
    int1 = 10;
    int2 = 20;

    UA_Int32* intArray = (UA_Int32*)UA_Array_new(2, &UA_TYPES[UA_TYPES_UINT32]);
    intArray[0] = int1;
    intArray[1] = int2;
...
printf("\n ouh yeah testArrayFunction      3.5       \n");

UA_Variant_setArray(&vattr.value, intArray, 2, &UA_TYPES[UA_TYPES_UINT32] );
    vattr.valueRank = UA_VALUERANK_ONE_DIMENSION;
    UA_UInt32 arrayDims[1] = {2};
    vattr.arrayDimensions = arrayDims;
    vattr.arrayDimensionsSize = 1;
    vattr.displayName = UA_LOCALIZEDTEXT("locale","ARRAY TEST");
    vattr.dataType = UA_TYPES[UA_TYPES_UINT32].typeId;
    printf("\n ouh yeah testArrayFunction      3.75       \n");

    UA_StatusCode retval = UA_Server_addVariableNode(server,
                           UA_NODEID_NUMERIC(1, 888999888), // new node id
                           UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER), // parent node
                           UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES), // reference type
                           UA_QUALIFIEDNAME(1, "UNION TEST!!!"), // node browse name
                           UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
                           vattr, NULL, NULL);
    if(retval == UA_STATUSCODE_GOOD){
        printf("\n testArrayFunction succesfully added!! \n");
    }else{
        printf("\n testArrayFunction adding failed!! \n");
    }
}
...
int main(int argc, char** argv) {
  ...
  //      testUnionFunction(server);
  //      testArrayFunction(server);
